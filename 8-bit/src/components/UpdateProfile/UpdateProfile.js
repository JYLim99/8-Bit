...
const UpdateProfile = () => {
  ...
  //Various functions to display username, status & image
  ...
  //Function to handle submitting of edit profile, updates the profile with any changes
  async function handleSubmit(event) {
    ...
    if (image !== '') {
      const uploadImage = storage
        .ref(`ProfilePictures/${currentUser.uid}/UserDisplayPic`)
        .put(image)
      uploadImage.on(
        'state_changed',
        (snapshot) => {},
  ...
  //Returns user to Dashboard after changes
  const handleReturn = () => {
    history.push('/Dashboard')
  }
    ...
    return (
      <div className={styles.background}>
        <form className={styles.profileContainer}>
          ...
          <button className={styles.dashboardButton} onClick={handleReturn}>
